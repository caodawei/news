...
<%@ page import="com.news.service.TopicService" %>
<%@ page import="com.news.service.impl.TopicServiceImpl" %>
            ...
            xhr.onreadystatechange = function () {
                if (xhr.status == 200 && xhr.readyState == 4) {
                    if (xhr.responseText == "ok") {
                        document.getElementById("lblUname").style.display = "none";
                        document.getElementById("lblUpwd").style.display = "none";
                        document.getElementById("uname").style.display = "none";
                        document.getElementById("upwd").style.display = "none";
                        document.getElementById("submit").style.display = "none";

                        document.getElementById("error").innerHTML = "";
                        document.getElementById("msg").innerHTML = "欢迎：" + uname;
                        document.getElementById("span").style.display = "inline";

                    } else {
                        document.getElementById("lblUname").style.display = "inline";
                        document.getElementById("lblUpwd").style.display = "inline";
                        document.getElementById("uname").style.display = "inline";
                        document.getElementById("upwd").style.display = "inline";
                        document.getElementById("submit").style.display = "inline";

                        document.getElementById("error").innerHTML = "用户名或密码错";
      ...
      <ul class="class_date">
          <li id='class_month'>
             <c:forEach items="${topics}" var="topic" >
              <a href='/news?tid=${topic.tid}'><b> ${topic.tname} </b></a>
              </c:forEach>
          </li>
...
<%--        <li id='class_month'> <a href='#'><b> 房产 </b></a> <a href='#'><b> 家居 </b></a> <a href='#'><b> 旅游 </b></a> <a href='#'><b> 文化 </b></a> <a href='#'><b> 其他 </b></a> </li>--%>
      ...
      <ul class="classlist">
                <c:forEach items="${newsList}" var="news" varStatus="status">
                    <li><a href='/news?action=detail&nid=${news.nid}'> ${news.ntitle}</a><span> ${news.ncreateDate}</span></li>
...
<%--        <li><a href='#'> 步行者崩盘主要原因在哪 解决3问题能发起更强挑战</a><span> 2013-06-06 01:01:47.0 </span></li>--%>
<%--        <li class='space'></li>--%>
...
<%--        <li><a href='#'> 我军少将：中印两国不会因领土争端大打出手 </a><span> 2013-06-06 10:54:54.0 </span></li>--%>
        <p align="right"> 当前页数:[${pager.currentPage}/${pager.totalPages}]&nbsp;
            ...
            <a href="/news?page=1${qs}">第一页</a>
            <a href="/news?page=${pager.previousPage}${qs}">上一页</a>
            </c:if>
            <c:if test="${pager.currentPage<pager.totalPages}">
            <a href="/news?page=${pager.nextPage}${qs}">下一页</a>
            <a href="/news?page=${pager.lastPage}${qs}">末页</a>
            </c:if>
        </p>
      </ul>
    </div>
